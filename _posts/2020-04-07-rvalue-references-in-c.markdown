...
> rvalues can be realized as everything that is not an l-value=========================================================Myclass g(UserClass()) // Here UserClass() is an rvalue  
=========================================================int i = 1; // 1 is an rvalue  
=========================================================int j = i--; // Here i-- is an rvalue=========================================================
> An rvalue has no name as its a temporary value.### Lvalue references
...
![](/img/1*yTSGKUlIEfiH9tZA7Akbsg.png)**The thing about lvalue references (Table#1 and Table#2)**=========================================================  
 ...
 int x{};  
 const int y = 0; // or const int x {0};=========================================================// **l-value references** **(Table 1)**  
 int &ref1{ x }; // OK  
 int &ref2{ y }; // Error  
 int &ref3{ 5 }; // Error=========================================================  
 ...
 const int &ref6{ 5 }; // OK  
 int &ref7{ 5 }; // Error=========================================================### Rvalue references
...
> ‘&&’ may also imply forwarding (a.k.a) universal reference. Will cover Universal references in another article.*![](/img/1*P44tpDonN4ti5N7Bvfaoog.png)**The thing about rvalue references (Table#3 and Table#4)**=========================================================int x{};  
const int y = 0; // or const int x {0};=========================================================**// r-value references (Table 3)  
** int &&ref7{ x }; // Error  
 int &&ref8{ y }; // Error  
 int &&ref9{ 5 }; // OK  
 ref9 = 6; // OK=========================================================**// r-value references to const (Table 4)**  
...
> Rvalue references can only bind to Rvalues.### What’s the point of rvalues?
...
> 2. Taking ownership of an object**### How to get an rvalue reference from anything?
...
> std::move is used to convert to rvalue reference, it is exactly equivalent to a static\_cast to an rvalue reference.=========================================================int x = 1, int y = 1;int &&ref1 = x; // Error, rvalue can't bind to lvalueint &&ref2 = **std::move**(x); **// This works fine**  
int &&ref3 = **static\_cast<int&&>**(y) // **Also works fine**### Usage of rvalues and rvalue references
...
**Rule 3: **When you write a class with the copy constructor and the assignment operator, write also the move constructor and the move assignment operator.  
Feel free to refer to [**this article**](https://medium.com/pranayaggarwal25/move-semantics-269e73287b63) for more details.
...
> A named rvalue is an lvalue as all Named objects are lvalues. Only temporary / anonymous objects are rvalues=========================================================void f(vector<int> && x) **// x is an rvalue reference**  
{  
 // ....  
 // here x is an lvalue since it designates the name of an object  
}
> **There can be rvalue references that are themselves lvalues!**It is common to misunderstand x as a rvalue reference when using it inside a function body like above. Here inside function bodyx itself is not an rvalue reference object because it has a name and named rvalue is an lvalue.  
...
> Values return by functions/methods and expression are temporary values, so you do have to use **std::move** to move them **(C++ standard to convert to rvalue)** when you pass them to functions/methods that take an rvalue reference as an argument.Take a look at the below code for example —
...
{  
 ...
 int &&ref{ 5 };   
 fun(ref);// **calls lvalue version of function!!! we didn't move it** fun(std::move(ref));// calls r-value version of function return 0;  
}  
=========================================================### References
